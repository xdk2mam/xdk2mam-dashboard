...
var DataService = function () {
    ...
    dataRepository
      ...
      .then(async function (data) {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          let root = null
          if (info.on_tangle === 1)
            root = await bundleRepository.getRoot(info.bundle_id)
  ...
  this.publish = async function () {
    ...
    if (dataset !== undefined) {
      ...
      if (dataset.dataset_interval <= 1000)
        limit = 10
      else if (dataset.dataset_interval <= 5000)
        limit = 5
      else if (dataset.dataset_interval <= 10000)
        limit = 2
      ...
      deferred.reject({ errorMessage: 'No active dataset was found.' })
