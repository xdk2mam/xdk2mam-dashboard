  ...
  this.putData = function (dataSensors) {
    ...
    connectionService.getConnectionRequest(query, function (err, data) {
      if (err) {
        ...
        if (data.length === 0) {
          console.log('No active dataset, please create one to insert data from the sensor.')
          return
        }
    ...
    connectionService.getConnectionRequest(query, function (err, data) {
      if (err) {
        ...
        if (data.length > 0) {
          ...
          else if (intervalSec === 0) queryInterval = ' '
          else queryInterval = ' AND on_tangle = 0'
          ...
          connectionService.getConnectionRequest(querySelect, function (err, dt) {
            if (err) {
              ...
              if (dt.length > 0) deferred.resolve(dt)
              else deferred.reject({ errorMessage: 'No data was found.' })
            }
          })
        } else deferred.reject({ errorMessage: 'No data was found.' })
      }
