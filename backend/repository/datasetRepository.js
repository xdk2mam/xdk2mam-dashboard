...
var DatasetRepository = function () {
    this.create = function (dataset) {
        ...
        query = "CREATE TABLE IF NOT EXISTS `"+dataset.Name+"` (" +
        ...
        connectionService.getConnectionRequest(query, function (err, data) {
            if (err) {
                ...
                const queryInsert = "INSERT INTO `datasets`"+
                "(`dataset_name_table`, `dataset_desc`, `status`, `dataset_start`, `dataset_end`, `dataset_interval`) VALUES"+
                "('"+dataset.Name+"', '"+dataset.Desc+"', 1, "+timestamp+","+dataset.DatasetEnd+", "+dataset.DatasetInterval+");"
    ...
    this.terminate = function (dataset) {
        ...
        query = "UPDATE `datasets` SET "+
            "`status` = 0 , `dataset_end` = "+timestamp+" WHERE `id` = "+dataset.id+";"
